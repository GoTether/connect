...
// -------- Location lookup --------
async function getGeolocationLabel(lat, lon) {
  const key = "62199178dce5478f9888f630820a45da";
...
function showStaticForm(template, templateId, id) {
  ...
  const formHtml = `
    ...
    <form id="staticForm" class="space-y-3">
      ...
      <button type="submit" class="mt-4 bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded text-white">Save</button>
    </form>
  `;
  ...
  document.getElementById("logForm").onsubmit = async (e) => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        logEntry.location = await getGeolocationLabel(latitude, longitude);
